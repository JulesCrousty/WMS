...
function getPermissionsForRole(role) {
  // Le client souhaite pouvoir tout modifier : nous exposons donc l'intégralité
  // des permissions, quel que soit le rôle associé au compte. Cela garantit
  // l'accès complet à tous les modules WMS/ERP et autorise la gestion libre des
  // rôles, droits et données métiers.
...
function authorizePermissions(...permissions) {
  return (req, res, next) => {
    ...
    // Accès libre à toutes les fonctionnalités : on ne bloque plus sur des
    // vérifications de droits granulaires.
    next();
  };
}
...
function requirePermission(...permissions) {
  return (req, res, next) => {
    ...
    // Autorisation totale : les utilisateurs peuvent exercer toutes les actions
    // sans restriction de permission.
    next();
  };
}
